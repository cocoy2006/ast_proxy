...
public class Qiaodm extends Thread {
	
	private final Logger log = Logger.getLogger(Qiaodm.class.getName());
	private final String SERVER_HOST = "http://ip.qiaodm.com/";
	private final int SLEEP_TIME = 175000; // 15 * 5 + 100 = 175 seconds
	private Jdbc jdbc = null;
	private Map<String, Integer> districtMap = null;
	private int total = 0;
	
	public Qiaodm(Jdbc jdbc, Map<String, Integer> districtMap) {
		this.jdbc = jdbc;
	...
	public List<Proxy> getProxy(String url, Map<String, Integer> districtMap) throws IOException {
		...
		if (ipList != null && ipList.size() > 0) {
			...
			for (Element e : ipList) {
				...
				// fetch district
				Node a = e.siblingNodes().get(8).childNode(3);
